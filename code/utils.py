...
def add_scaled_glcm(image, roi, scale, high_val):
    ...
    min_max_val = glcm_image.reduceRegion(geometry = roi, reducer = ee.Reducer.minMax(), scale = scale, maxPixels = 100000000000)
...
def prepare_terrain_seg(terrain_image, roi, scale, high_val):
    terrain_minMax = terrain_image.select('slope')\
        .reduceRegion(ee.Reducer.minMax(), geometry = roi, scale = scale)
