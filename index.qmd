...
The methodology uses an OBIA landTrendr approach applied to annual class probabilities. 
   ...
   1. Data Acquisition and Preprocessing: GEE and python will be used for all the analysis if possible to keep the workflow easily repeatable and well documented.
      ...
      - Datasets: Landsat (2000-2020/2024). Need to calculate annual metrics (mean, median, growing season, etc.) for each year. For example, assuming mean NDVI as a metric, if there are 25 images in 2005, mean NDVI will be calucated pixelwise and there will be a mean NDVI image for 2005. This is repeated for each year and each selected metrics. These metrics will be fed into the RF to calculate agricultural land probability.
  ...
  2. Create agricultural land objects: The study will be carried out at object level to reduce "salt and pepper"  noise common in pixel-based methods. The multi-resolution segmentation or similar algorithm will be applied to stacked Landsat images to create agricultural land objects [@yinMappingAgriculturalLand2018]. Instead of using a single image for segmentation, [@yinMappingAgriculturalLand2018] applied the algo to stack of landsat images (Aug 1989, Sep 1998, May 2007 and Jul 2015) to create temporally homogeneous objects. They used `"bottom-up multi-resolution segmentation algorithm"` in eCognition. This approach initiates each pixel as a single segment and merges spatially adjacent segments based on their similarity until the desired scale is met.
  
      - Alternative 1: Use existing agricultural land from land cover datasets or osm or similar like [@wuMonitoringCroplandAbandonment2023]. Consider all the cropland from 2000-2022 as agricultural land objects or filter out values greater than 2/3 to filter out some error/noise.

  3. Calculate Annual Provbability (probability of an object being agricultural land): [@daraMappingTimingCropland2018; @yinMappingAgriculturalLand2018] used "stable" training sites (areas that did not change ). They extracted the spectral signature for those stable sites across multiple years to train the model. Ensure the model is robust enough to handle inter-annual phenological variations and recognize farmland under varying climatic conditions (wet vs dry years). [@daraMappingTimingCropland2018] says `"Abandoned areas spectrally align along a gradient between cropped and uncropped areas. This is represented well in class probabilities of cropland vs. non-cropland classes (Yin et al., 2014, 2018). We thus derived cropland probabilities between 1985 and 2015 which then served as input for a trajectory-based approach to detect abandonment and recultivation events over time. This allowed omitting the high inter-annual spectral changes of cropland at the pixel level. Another advantage of using probabilities is their robustness against mixed pixels in the land cover class of interest (Colditz et al., 2011; Yin et al., 2014)."`

  4. Apply landTrendr to Time Series Probabilities: Feed the annual probalities values into the landTrendr algorithm. This step converts a noiy stact of annual classification data into smooth, logical history of land use for eavy agricultural object. When landTrendr is applied to the annual probability values, it performs two operations:
      - Noise Removal (Despiking): Annual probabilities often fluctuate due to weather events (eg, drought) or image artifats. landTrendr apploes a "despiokin" parameter to ignore the short-term outliers that do not represent ta permanent land-use change. 
      - Curve Fitting (Segmentation): The algorithm attempts to fit a series of straight lines (segments) to the probability history. It simplifies the complex year-to-year data into stable periods (plateaus) and changing periods (transitions)

  5. Identify Structural Breaks and Classify Each Object into "AAL", "Recultivated", or "Fallow": The core goal of this step is to find the vertices (breakpoints) between these segments. A "structural break" indicates the exact year the land management regime changed. This is the specific year where the trajectory shifts from a high probabilty of farming to a declining or low probability. The algorith calculates the magnitude of the change. For abandonment we look for a transition where the pre-change value is high (like >0.5) and the post change value is low (like <0.5). Finding this drop is not enough, we must distinguish abandonment from fallow (temprorary). this is done by applying a duration logic to the landTrendr segments. Following the methodologies of [@yinMappingAgriculturalLand2018] and [@daraMappingTimingCropland2018], we can classify an object as "Abandoned" only if the probability drops and remains low for 5 years.
        - Fallow: If the probability drops but recovers within 5 years, it is labeled as "Fallow" or "Rotation" rather than abandonment.
        - Recultivation: If the probability drops for >5 years but then rises again near the end of the time series, the it is labelled as "Recultivation"

  6. Validation: Overall Accuracy, Kappa Coefficient, Producer's Accuracy, User's Accuracy
...
::: {.callout-note collapse="true"}
## Earlier plan for sample collection that I abandoned due to resource contraints 

Notes on Sample Data collection:
I am adopting @daraMappingTimingCropland2018 to train random forest classifier. I will be using single constant training dataset to estimatge cropland probabilities across entire period.

- Target Classes: We must exclusively collect samples from area that never changed land cover status. Stable cropland referes to areas premanently cropped between 2000 and 2035. While cropland may experience short seasonal fallows, the 3-year temporal moving window applied to the landsat metrics will smooth out this short-term noise. Stable non-cropland refers to areas that were never cropped during the study period, such as permanent forests, rangelands, or water bodies.

- Collecting Training Samples:
  - Use the existing land cover dataset to find the potential stable agricultural areas.
  - generate random points inside those areas
  - visually inspect each of the point to identify true stable agricultural areas.
  - Do the same for non-agricultural areas.
:::

### Notes on Sample Data collection:
I was going to adopt the @daraMappingTimingCropland2018 for sample data collection but it seemed too time consuming for a single person to collect over 4000 samples looking at each each historial image from Google Earth. Therefore, I have decided to use the method by @alcantaraMappingExtentAbandoned2013 to collect samples. Like they did, I am going to use the existing land cover dataset to collect samples. Similar to @alcantaraMappingExtentAbandoned2013, @wuMonitoringCroplandAbandonment2023 also used existing datasets for identifying agricultural lands. 

Based on @alcantaraMappingExtentAbandoned2013, I will collect the samples using the following process:
...
8. After this I can continue following @daraMappingTimingCropland2018 and @yinMappingAgriculturalLand2018


### Notes on phenological check
I will be using the phenological signature of the potential stable cropland NDVI values from 2000-2022 to identify the stable active cropland samples.

#### Profile of Active Cropland
...
#### The Profile of Stable Non-Cropland or Fallow Land (What to Discard/Relabel): 
...
Based on the specific phenological characteristics of crops in the mountainous regions of Nepal established by Mishra et al. (2023), the peak detection algo in scipy needs to be following.

Because I am using an 8-day Landsat composite (resampled to 16 Days) and find_peaks algo operates on (samples), not days. Therefore, 1 sample = 16 days.
**Parameters for 8 days composite and 16 days resampled**
1. height: 0.4/0.35
Literature value: Mishra et al. determined that for a peak to be considered an active crop in Nepal, the maximum NDVI must be above 0.4. Start with height=0.4, but because the 16-day mean might depress the absolute peak value, I may need to lower this slightly (e.g., to 0.35) if the function starts missing obvious crop cycles.

2. distance: 15/7/8
Literature value: The authors established that the duration between a local minimum (planting/plowing) and a local maximum (peak growth) for a valid crop cycle in Nepal ranges from 60 to 120 days. This means a full cycle (from one peak to the next peak in a multi-cropping system) takes at least 120 days.
How to set it: 120 days ÷ 8 days/sample = 15 samples. Set distance=15. This prevents the function from overcrowding your data by falsely identifying two peaks within the same 120-day growing season.
Since 1 sample now equals 16 days after resampling, must adjust the mathematical thresholds defining a crop cycle (120 to 240 days):
distance: The minimum time between two crop peaks is roughly 120 days.
Calculation: 120 days ÷ 16 days/sample = 7.5.
New Value: Set distance=7 or distance=8.

3. width: (15, 30)/(7, 15)
Literature value: As stated above, a single crop cycle (from minimum to minimum, or base to base) takes roughly 120 to 240 days (which is 60 to 120 days from min to max, multiplied by two).
How to set it: 120 days is 15 samples, and 240 days is 30 samples. Setting width=(15, 30) ensures the algorithm only flags peaks that have the specific "narrow" temporal width of an agricultural crop, filtering out the wide, bell-shaped curves of natural forests. For 16 day resampled: 120 ÷ 16 = 7.5. 240 ÷ 16 = 15.

New Value: Set width=(7, 15) or width=(8, 15).
**A Crucial Clarification on the "60-Day" Literature Rule** 
According to the phenological rules established by Mishra et al., the 60 to 120-day timeframe refers strictly to the duration between the local minima (planting/land preparation) and the local maxima (peak vegetative growth). It does not represent the full duration of the crop in the field. Because it takes 60 to 120 days just to reach peak growth, a full crop cycle (from the bare-soil minimum of planting to the bare-soil minimum of the harvest) actually takes 120 to 240 days.

Therefore, for standard crops (Rice, Maize, Wheat): For targeting the standard staple crops that follow Mishra et al.'s 120-to-240-day full cycle rule, width=(7, 15). A minimum width of 7 samples equals 112 days (7 × 16), which perfectly catches the 120-day full cycles without missing them.

For fast-growing crops: 60-day short-season catch crops (like quick-growing vegetables), width=(3, 15) so the algorithm accepts peaks that are only ~3.75 samples wide. However, be aware that lowering the width this much on a 16-day smoothed composite significantly increases the risk of the algorithm falsely counting a 1-month flush of weeds as an active crop cycle.

3. prominence: 0.15/0.12 (Estimated from thresholds)
While the authors do not give a single "prominence" metric, they mandate a "consistent NDVI increment after the detected minima" and note that the average annual NDVI for these active fields is typically below 0.25. If the baseline (minima) hovers around 0.25 and the peak (maxima) must be at least 0.40, the crop "stands out" from its baseline by at least 0.15. Therefore, prominence=0.15. This is highly effective for your cloud-interpolated NDVI data, as it forces the algorithm to only count peaks that rise significantly from the bare-soil baseline, ignoring minor fluctuations. Because the 16-day average will smooth out the sharp, sudden drops of bare soil during harvest, the trough-to-peak difference will shrink. Start with prominence=0.15, but be prepared to lower it to 0.10 or 0.12 to ensure the algorithm still detects peaks that don't rise as sharply from the smoothed baseline.

#### Operating on your 'smoothed_ndvi' 16-day array
peaks, properties = find_peaks(
    smoothed_ndvi, 
    height=0.4,         # May need to be lowered from 0.4 due to 16-day averaging
    distance=7,          # 7 samples * 16 days = 112 days (approx. 120 day minimum)
    width=(7, 15),       # Crop cycle lasts between 112 and 240 days
    prominence=0.15      # Lowered slightly to account for the flattened troughs
)

### Notes on training sample generation
After getting the consensus stable cropland, erosion and expansion and spatial filtering:
 1. Phenology filtering was done with a combination of peak detection and phenology identification algo.
 2. After that, 10% of the phenology verified points were taken for validation through google earth historical imagery.  
 3. Then i checked the accuray of the phenology filtering and found it to be around 75-80% in the beginning. 
 4. To improve the accuracy, I checked how the NDVI values in the validation samples of rangeland and stable ag differred in terms of mean, max, min median. After a couple of hit and trail. I found:
 Stable cropland: Mean Min NDVI	0.159 and Mean Amplitude	0.650

 Non-stable rangeland: Mean Min NDVI 0.198 Mean Amplitude 0.568
 Therefore if overall_amplitude < 0.55 or overall_min > 0.25 discard as rangeland. This worked and my accuracy reached 83%.

 5. I did not have to do the same for the stable non-crop because I got 98% accuracy on the first try. I anly used the land cover information to identify the relationship for different classes. Still I had to hit and trial for the right threshold.
### Note on Stable Non-Crop Phenology Filtering Adjustment
An initial strict phenology filter yielded 1,282 stable non-crop points, which achieved a 97% accuracy via manual visual interpretation of a 10% random sample in Google Earth. However, this strict thresholding systematically discarded natural rangelands due to their low winter NDVI baseline. To correct this under-representation, we applied a 5th/95th percentile adjustment to the phenology bounds (overall_min = quantile(0.05) and overall_max = quantile(0.95)) and relaxed the valid observation requirement from 200 to 150 for cloud-prone regions. This successfully rescued an additional 1,274 points (predominantly rangelands and sparse vegetation). To ensure this algorithmic adjustment did not introduce agricultural false positives, we conducted a targeted validation on a random sample of the rescued points, achieving a visual accuracy of XX%. The combined, verified dataset of 2,556 points was then utilized for model training.

### Note on SNIC segmentation
Images were acquired for following dates.
1. Post-Monsoon (October–November) Rainfall is significantly reduced and skies begin to clear during this period
. Acquiring imagery during these months is critical for capturing the maturity and harvest stages of Nepal's primary summer crops, such as main season rice and millet, which are typically planted between June and August and harvested between October and November
.
2. Winter (December–February) This season offers the most reliable conditions for optical remote sensing in Nepal due to clear skies and very little precipitation
. Images from these months will allow the segmentation algorithm to delineate fields used for winter crops, such as wheat, potato, and buckwheat, which are planted from November to January and harvested into the spring
.
3. Pre-Monsoon (March–May) While the weather becomes hotter with some scattered rainfall, this window is necessary to capture the boundaries of spring crops
. Maize and spring rice are typically planted between February and April and harvested in June or July
.